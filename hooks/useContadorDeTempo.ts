export const useContadorDeTempo = (data: Date | string): string => {
    ...
    const diferencaEmMilissegundos = dataAtual.getTime() - dataInformada.getTime();
    ...
    if (diferencaEmMeses < 12) {
        return diferencaEmMeses === 1 ? "1 mês atrás" : `${diferencaEmMeses} meses atrás`;
    }

    const diferencaEmAnos = Math.floor(diferencaEmMeses / 12);
    return diferencaEmAnos === 1 ? "1 ano atrás" : `${diferencaEmAnos} anos atrás`;
};
